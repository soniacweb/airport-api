...
app.get("/airports/:id", (req, res) => {
    ...
    const singleAirport = airports.find(
        (airport) => airport.icao === singleAirportId
    );
...
app.put("/airports/:id", (req, res) => {
    ...
    const newAirportData = req.body; // what you're passing to the endpoint
    let indexToReplace = airports.findIndex(
        (airport) => airport.icao === singleAirportId
    ); // first id that matches with the searched id
    airports[indexToReplace] = newAirportData; // reassigning the updated airport
    console.log(airports[indexToReplace]);
    res.sendStatus(200);
    // res.sendStatus(201);
});
// delete an airport
